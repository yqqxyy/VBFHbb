...
class FitManager : public FitProcessor {
 public:
   ...
   */
  bool Initialize(std::string datadir, std::string year);
   ...
   * @return Standard error flag
   */
  bool AddParticle(const ParticleType::Particle& type, const std::string& name,
                   const double& pt, const double& eta, const double& phi,
                   const double& E, const int& userIndex = -1);
  ...
  bool FixFitParameter(ParameterType::Parameter paramType,
                       std::string particleName, int particleIndex = -1);
