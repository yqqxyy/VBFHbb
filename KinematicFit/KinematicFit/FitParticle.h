...
class FitParticle {
 public:
   ...
   */
  inline void SetIndex(int index) { fIndex = index; }

  /**
   * Set global user index of particle (if needed)
   */
  inline void SetUserIndex(int index = -1) { fUserIndex = index; }
   ...
   */
  inline void SetType(ParticleType::Particle type) { fParticleType = type; }

  /**
   * Set type position of particle in container
   */
  inline void SetTypePosition(int pos) { fTypePosition = pos; }
  ...
  bool AddFitParameter(const ParameterType::Parameter& paramType,
                       const double& iniVal, const double& paramRes,
                       const double& stepSize, const double& lowerLimit,
   ...
   */
  void SetParamMinuitIndex(int index, int minuitIdx) {
    fParamMinuitIndex[index] = minuitIdx;
  }
   ...
   */
  inline int GetUserIndex() { return fUserIndex; }
   ...
   */
  inline ParticleType::Particle GetType() { return fParticleType; }

  /**
   * @return Type position of particle in container
   */
  inline int GetTypePosition() { return fTypePosition; }

  /**
   * @return Fit configuration by index
   */
  inline ParameterType::Parameter GetParameterType(int index) {
    return fFitParameter[index];
  }
  inline double GetParamInitialValue(int index) {
    return fParamInitialValue[index];
  }
  inline double GetParamResolution(int index) {
    return fParamResolution[index];
  }
  inline double GetParamResolutionSquared(int index) {
    return fParamResolutionSquared[index];
  }
  inline double GetParamStepSize(int index) { return fParamStepSize[index]; }
  inline double GetParamLowerLimit(int index) {
    return fParamLowerLimit[index];
  }
  inline double GetParamUpperLimit(int index) {
    return fParamUpperLimit[index];
  }
   ...
   */
  static bool PT_ORDER(FitParticle* lhs, FitParticle* rhs) {
    return (lhs->fPt > rhs->fPt);
  }
  static bool E_ORDER(FitParticle* lhs, FitParticle* rhs) {
    return (lhs->fE > rhs->fE);
  }
  static bool ETA_ORDER(FitParticle* lhs, FitParticle* rhs) {
    return (std::fabs(lhs->fEta) < std::fabs(rhs->fEta));
  }
  static bool PHI_ORDER(FitParticle* lhs, FitParticle* rhs) {
    return (std::fabs(lhs->fPhi) < std::fabs(rhs->fPhi));
  }
