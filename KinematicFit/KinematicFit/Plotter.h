...
class Plotter {
 public:
  ...
  std::vector<std::pair<TString, Histogram*> >
      fHistTypeContainer;  //(TypeName, TypeObject)

  //Histogram Data Container
  std::vector<std::pair<TString, TList*> >
      fHistDataContainer;  //(DataType, HistDataObject)
  ...
  //1D
  void DefineHistogramType(TString histtype, TString xtitle, Int_t xbins,
                           Float_t xlow, Float_t xhigh, TString ytitle);
  //2D
  void DefineHistogramType(TString histtype, TString xtitle, Int_t xbins,
                           Float_t xlow, Float_t xhigh, TString ytitle,
   ...
   */
  bool FillTree(TString treename, double var, double pt, double eta, double phi,
                double E, double weight);

  //Fill histogram (and dynamically add if missing, given an existing data and hist type)
  bool Fill(TString datatype, TString histtype, TString histname, double value,
            double weight);
  bool Fill(TString datatype, TString histtype, TString histname, double xvalue,
...
//___________________________________________________________________________________
inline void Plotter::DefineHistogramType(TString histtype, TString xtitle,
                                         Int_t xbins, Float_t xlow,
                                         Float_t xhigh, TString ytitle) {
  ...
  if (int(fHistTypeContainer.size()) != 0) {
    for (unsigned int i = 0; i < fHistTypeContainer.size(); i++) {
      if (fHistTypeContainer[i].first == histtype) {
        std::cout << "Plotter: ( WARNING ) \t histtype " << histtype
...
//Fill 4-vector tree
//___________________________________________________________________________________
inline bool Plotter::FillTree(TString treename, double var, double pt,
                              double eta, double phi, double E, double weight) {
  Tree* myTree = NULL;
...
//Fill histogram (and dynamically add if missing, given an existing data and hist type)
//___________________________________________________________________________________
inline bool Plotter::Fill(TString datatype, TString histtype, TString histname,
                          double value, double weight) {
  //Check if hist types are available
  if (int(fHistTypeContainer.size()) == 0) {
    std::cout << "Plotter: ( ERROR ) \t No hist types defined. Abort!"
              << std::endl;
    return false;
  }
  ...
  if (histtype_Idx < 0) {
    std::cout << "Plotter: ( ERROR ) \t Requested histtype " << histtype
              << " for filling not found. Abort!" << std::endl;
    return false;
  }  //exception
...
//___________________________________________________________________________________
inline TH1F* Plotter::Get1DHistogram(TString datatype, TString histname,
                                     Int_t histtype_Idx) {
  TH1F* tmp = NULL;

  //Sanity check
  if (histtype_Idx < 0) {
    std::cout << "Plotter: ( ERROR ) \t histtype_idx is negative. Abort!"
              << std::endl;
    return tmp;
  }
  ...
  if (fHistTypeContainer[histtype_Idx].second->is1D) {
    ...
    tmp = new TH1F(TString(datatype + "_" +
                           fHistTypeContainer[histtype_Idx].second->fType +
                           "_" + histname),
                   fHistTypeContainer[histtype_Idx].second->fType,
    ...
    std::cout << "Plotter: ( ERROR ) \t Requested histtype is not 1D. Abort!"
              << std::endl;
  }
...
//___________________________________________________________________________________
inline bool Plotter::Fill(TString datatype, TString histtype, TString histname,
                          double xvalue, double yvalue, double weight) {
  //Check if hist types are available
  if (int(fHistTypeContainer.size()) == 0) {
    std::cout << "Plotter: ( ERROR ) \t No hist types defined. Abort!"
              << std::endl;
...
//___________________________________________________________________________________
inline TH2F* Plotter::Get2DHistogram(TString datatype, TString histname,
                                     Int_t histtype_Idx) {
  TH2F* tmp = NULL;
  ...
  if (fHistTypeContainer[histtype_Idx].second->is2D) {
    /*//Temp. Hack for custom binning for eta and pt
      ...
      etaBins);
    */
    tmp = new TH2F(TString(datatype + "_" +
                           fHistTypeContainer[histtype_Idx].second->fType +
                           "_" + histname),
                   fHistTypeContainer[histtype_Idx].second->fType,
                   fHistTypeContainer[histtype_Idx].second->fNXBins,
    ...
    std::cout << "Plotter: ( ERROR ) \t Requested histtype is not 2D. Abort!"
              << std::endl;
  }
...
inline void Plotter::WriteOutput() {
  ...
  if (int(fHistDataContainer.size()) == 0) {
    std::cout << "Plotter: ( ERROR ) \t No data found to write to file. Abort!"
              << std::endl;
    return;
  }
...
inline void Plotter::FindRange(TString name, double value) {
  ...
  for (unsigned int i = 0; i < fVariableContainer.size(); i++) {
    if (fVariableContainer[i].first == name) {
      if (value < fVariableContainer[i].second->fMin)
        fVariableContainer[i].second->fMin = value;
      if (value > fVariableContainer[i].second->fMax)
        fVariableContainer[i].second->fMax = value;

      fVariableContainer[i].second->fN++;
      fVariableContainer[i].second->fSum += value;
      fVariableContainer[i].second->fMean =
          fVariableContainer[i].second->fSum / fVariableContainer[i].second->fN;
      return;
    }
  }
