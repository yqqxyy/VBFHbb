...
class KinematicFit {
 public:
   ...
   */
  bool Init(std::string datadir = "", std::string year = "2012",
            bool isMC = true);
   ...
   * @return Standard error flag
   */
  bool AddSoftTerm(const std::string& name, const TLorentzVector& tlv,
                   const float& PRes);
  bool AddMuon(const std::string& name, const TLorentzVector& tlv,
               const float& PRes);
  bool AddMuon(std::string name, float pt, float eta, float phi, float E,
               float PRes);
   ...
   */
  bool AddMuon(std::string name, float pt, float eta, float phi, float E,
               float qp_cov, MuonType::Detector type);
   ...
   * @return Standard error flag
   */
  bool AddElectron(const std::string& name, const TLorentzVector& tlv,
                   const float& ERes);
   ...
   * @return Standard error flag
   */
  bool AddJet(const std::string& name, const TLorentzVector& tlv,
              const float& ERes, const std::string& JetType,
              const bool& semilep);
  bool AddJet(std::string name, float pt, float eta, float phi, float E,
              float ERes, std::string JetType, bool semilep);
  bool AddJet(std::string name, bool semilep, float pt, float eta, float phi,
              float E);
   ...
   */
  void GetFitFourVector(std::string name, float& pt, float& eta, float& phi,
                        float& E);
   ...
   */
  void GetFourVector(std::string name, float& pt, float& eta, float& phi,
                     float& E);
