  ...
  //m_overlapRemovalToolVBFGamma(nullptr),
{
  ...
  m_config.getif<bool>("applyRelPtMuJetOR",m_applyRelPtMuJetOR);
}
...
EL::StatusCode OverlapRemoval::initialize()
{
  ...
  TOOL_CHECK("OverlapRemoval::initialize()", m_toolBox.muJetORT.setProperty("ApplyRelPt", m_applyRelPtMuJetOR) );
  ...
  //Override Jet-fJet, Tau-jet OR property
  if( m_applyORlargeR ){
	// Jet-FatJet OR property
  	if( !m_doJetLargeJetOR ){
		...
		TOOL_CHECK("OverlapRemoval::initialize()", m_toolBox.jetFatJetORT.setProperty("DR", 1.0) );
	}
  }
