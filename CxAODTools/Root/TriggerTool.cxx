...
TriggerTool::TriggerTool(ConfigStore& config) :
      ...
      m_muonThresholdFactor(1.05),
      m_egammaThresholdAdd(1.0),
...
void TriggerTool::addLowestUnprescaledElectron() {
  ...
  ADD_TRIG_ONLYTHRESHOLD(HLT_e300_etcut,	 any,  data16A, data18);// Egamma experts agreed that no needs for trigger match for this trigger
...
std::set<TriggerInfo*> TriggerTool::getPassedTriggers() {
  ...
  for (TriggerInfo* trigger : m_triggers) {
    ...
    if ( trigger -> requiresThreshold()) {
      if (! passThreshold(trigger)) continue;
    }
...
bool TriggerTool::passThreshold(TriggerInfo* trigger) {
  TriggerInfo::TriggerObject object = trigger -> getTriggerObject();
  if (m_debug) {
    Info("TriggerTool::passThreshold()",
         "Trying to check trigger threshold for trigger '%s' and object type '%i'",
	 trigger -> getName().Data(),(int)object);
  }
  const float threshold = trigger -> getThreshold();
  bool passThreshold = false;
  if (object == TriggerInfo::TriggerObject::Electron) {
    for (const xAOD::Electron* ele : m_electrons) {
      if(ele -> pt() > (threshold + m_egammaThresholdAdd)) passThreshold = true;
    }
  } else if (object == TriggerInfo::TriggerObject::Photon) {
    for (const xAOD::Photon* pho : m_photons) {
      if(pho -> pt() > (threshold + m_egammaThresholdAdd)) passThreshold = true;
    }
  } else if (object == TriggerInfo::TriggerObject::Muon) {
    for (const xAOD::Muon* muo : m_muons) {
      if(muo -> pt() > threshold * m_muonThresholdFactor) passThreshold = true;
    }
  }
...
int TriggerTool::getRunNumber() {
  ...
  bool newrun = (runNumberFile != lastRunNumber);

  if (newrun || (runNumberFile==0)) {
    m_nRuns++;
...
double TriggerTool::getScaleFactorMuon(std::set<TriggerInfo*> triggers) {
  ...
  if (m_debug) {
    Info("TriggerTool::getScaleFactorMuon()",
            "Before Scale factor is %f for %lu valid muons and %lu passed triggers %s.",
            scaleFactor, m_muons.size(), triggers.size(), triggerCombo.Data());
  }
