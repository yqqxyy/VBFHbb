...
// Compiler script for adding a trigger _with_ threshold requirement:
#define ADD_TRIG_ONLYTHRESHOLD(name, dataType, firstPeriod, lastPeriod) \
  ADD_TRIG(name, dataType, firstPeriod, lastPeriod) \
  triggerInfo_##name -> setRequireThreshold(true);

// Compiler script for adding a trigger _with_ matching and related pT threshold:
...
protected:
  ...
  virtual bool passThreshold(TriggerInfo* trigger);
  ...
  float m_muonThresholdFactor;
  float m_egammaThresholdAdd;
