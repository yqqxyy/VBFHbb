...
int main(int argc, char* argv[]) {
  ...
  // steer file has to put here: CxAODMaker/data/framework-run.cfg
  // RootCore will copy it to $ROOTCOREBIN/data/CxAODMaker/framework-run.cfg
  // ConfigStore::createStore() will prepend "$ROOTCOREBIN/" to the path given
  std::string configPath = "data/CxAODMaker_VBFHbb/framework-run.cfg";
  if(overrideConfig) {
    ...
    for (unsigned int isam = 0; isam < sample_name_in.size(); ++isam) {
      ...
      for(unsigned int i = 0 ; i < str.length(); i++) {
        ...
        // keep the 1st _s... of reco
          ...
          else if (count_underscore==2) sample_out_rname += str[i];
        }
      }
      // prune generators
      ...
      else             sampleHandler.setMetaString (".*"+sample_out_name2+".*"+sample_out_rname+".*", "nc_outputSampleName", sample_out_begin+sample_out_prune+sample_out_rname+sample_out_end);
      std::cout << "HELLOWORLD " << (".*"+sample_out_name2+".*"+sample_out_rname+".*") << " ---- " <<  (sample_out_begin+sample_out_prune+sample_out_rname+sample_out_end) << '\n';
    }  // output job name
