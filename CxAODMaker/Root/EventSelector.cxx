...
EL::StatusCode EventSelector::initialize()
{
  ...
  if (m_doCloseByIsoCorr) {
    ...
    muWPs.push_back("PflowTight_VarRad");
    muWPs.push_back("PflowTight_FixedRad");
    muWPs.push_back("PflowLoose_VarRad");
    muWPs.push_back("PflowLoose_FixedRad");
    muWPs.push_back("HighPtTrackOnly");
    muWPs.push_back("TightTrackOnly_VarRad");
    muWPs.push_back("TightTrackOnly_FixedRad");
    muWPs.push_back("Tight_VarRad");
    muWPs.push_back("Tight_FixedRad");
    muWPs.push_back("Loose_VarRad");
    muWPs.push_back("Loose_FixedRad");
...
EL::StatusCode EventSelector::doIsoCorrection() {
  //Corrects isolation WP variables for nearby leptons.
  //Corrects leptons passing passPreSel_noIso, for nearby leptons passing passPreSel_noIso
  if (m_doCloseByIsoCorr) {
    xAOD::IParticleContainer leptons(SG::VIEW_ELEMENTS);
    for(auto pobj: *m_electrons->getInParticleVariation( "Nominal" )) if (Props::passPreSel_noIso.get(pobj)) leptons.push_back(pobj);
    for(auto pobj: *m_muons->getInParticleVariation( "Nominal" )) if (Props::passPreSel_noIso.get(pobj)) leptons.push_back(pobj);

    //correct isolation and propagate to signal deco for electrons
    for (const auto& electron : *m_electrons->getInParticleVariation( "Nominal" )) if (Props::passPreSel_noIso.get(electron))  {
      auto isoAccept = m_isoCloseByTool->acceptCorrected(*electron, leptons);
    ...
    //correct isolation and propagate to signal deco for muons
    for (const auto& muon : *m_muons->getInParticleVariation( "Nominal" )) if (Props::passPreSel_noIso.get(muon)) {
      auto isoAccept = m_isoCloseByTool->acceptCorrected(*muon, leptons);
      // WPs 21.2.111+
...
bool EventSelector::performSelection(const TString& sysName) {
  ...
  //mc16a PFlow crack electron veto
  //https://twiki.cern.ch/twiki/bin/viewauth/AtlasProtected/HowToCleanJetsR21#

  bool checkCrackEl = false;
  int isCrackVetoClean = 1;
    ...
    for(const auto& ele : *containers.electrons) {
      if (!ele->isAvailable<char>("DFCommonCrackVetoCleaning")){
        if(ele->caloCluster()){ 
	      ...
	      && fabs(ele->caloCluster()->etaBE(2))<1.52){
	    inCrack = true;
	  }
	}
      } else {
        if(!(ele->auxdata<char>("DFCommonCrackVetoCleaning"))) {
	  inCrack = true;
	}
      }
      if(checkCrackEl && inCrack && Props::forMETRebuild.get(ele)){
	isCrackVetoClean = 0;
      }
    }
  }
  if(containers.photons){
    for(const auto& photon : *containers.photons) {
      if (photon->isAvailable<char>("DFCommonCrackVetoCleaning")){
        if(checkCrackEl && !(photon->auxdata<char>("DFCommonCrackVetoCleaning")) && Props::forMETRebuild.get(photon)){
	isCrackVetoClean = 0;
        }
      }
    }
  }
  Props::isCrackVetoClean.set(m_info->getEventInfo(), isCrackVetoClean);
