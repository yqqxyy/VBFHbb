...
EL::StatusCode AnalysisBase::initializeMetadata() {
  ...
  Info("initializeMetadata", "Processing file '" + fileName + "'");

  // get the MetaData tree once a new file is opened, with
  TTree *MetaData = dynamic_cast<TTree*>(wk()->inputFile()->Get("MetaData"));
  if (!MetaData) {
    Error("initializeMetadata", "MetaData not found! Exiting.");
    return EL::StatusCode::FAILURE;
  }
  MetaData->LoadTree(0);
  ...
  if(m_isDerivation){
    ...
    if(m_derivationName == ""){
      Error("initializeMetadata","Failed to retrieve derivation name from MetaData! Exiting.");
      return EL::StatusCode::FAILURE;
    }
    if(!fileName.Contains(m_derivationName)){
      Error("initializeMetadata","Mismatch between derivation in Metadata and sample name! Exiting.");
      return EL::StatusCode::FAILURE;
    }
    Info("initializeMetadata","Running on derivation (%s)!",m_derivationName.c_str());
  }
  else Info("initializeMetadata","Running on xAOD!");
  ...
  if (m_isMC) {
    ...
    if (fileName.Contains(re)|| sampleName.Contains(re)) {
      Info("initializeMetadata","Running on fast-simulation sample!");
      m_isAFII = true;
    }else{
      Info("initializeMetadata","Running on full-simulation sample!");
    }
  }
  ...
  Info("initializeMetadata","Retrieved from sample (%s) the last p-tag as %s!",sampleName.Data(),m_pTag.c_str());
  if (m_pTag=="") {
    Error("initializeMetadata","Failed to retrieve pTag name from sampleName or fileName! Exiting.");
    return EL::StatusCode::FAILURE;
  }
  ...
  //check if it is top MC, use sampleName for the grid jobs
  std::vector<std::string> vecStr{"tt","top","tW","lepWpT","tchan","Wt","tH"};
  for (auto str : vecStr) {
    if (fileName.Contains(str) || sampleName.Contains(str)) {
      m_isTopMC = true;
    }
  }
  Info("initializeMetadata", "m_isTopMC = %i", m_isTopMC);
...
EL::StatusCode AnalysisBase::fileExecute() {
  ...
  if (!m_histEventCount) {
    ...
    EL_CHECK("fileExecute()", histInitialize());
  }
...
EL::StatusCode AnalysisBase::initializeHandlers() {
  ...
  m_doIsoCorrection = false;
  m_config->getif<bool>("doCloseByIsoCorr",m_doIsoCorrection);
...
EL::StatusCode AnalysisBase::execute() {
  ...
  if (m_doIsoCorrection) {
    //Correct isolation WP decorations, considering e/mu passing passPreSel_noIso
    EL_CHECK("execute()",m_selector->doIsoCorrection());
    //Rerun e/mu selection, using corrected isolation WPs
    EL_CHECK("execute()",m_electronHandler->select());
    EL_CHECK("execute()",m_muonHandler->select());
  }
  ...
  if (m_eventInfoHandler) {
    if (m_isMC) {
      if(m_truthWZjetHandler)     EL_CHECK("setTruthWZProps",m_eventInfoHandler->setTruthWZProps(m_truthWZjetHandler->getInParticleVariation("Nominal")));
    }
  }
  ...
  if (m_isMC && m_truthProcessor) {
    if(m_isTopMC){
      ...
      // computing this variable can cause crashes with some derivations (e.g. HIGG4D1 ttbar)
      m_config->getif<bool>("computeCodeTTBarDecay", computeCodeTTBarDecay); 
    ...
    // computing this variable can cause crashes with some derivations (e.g. HIGG4D1 ttbar)
    m_config->getif<bool>("findMuonsFromHadronDecays", findMuonsFromHadronDecays); 
  ...
  if (m_isMC && m_truthProcessor && m_truthProcessor->isNewStyle()) {
    ...
    EL_CHECK("execute()", m_eventInfoHandler->isVZcc( m_truthProcessor->checkSherpaVZqqZcc() ));
  }
...
EL::StatusCode AnalysisBase::cleanUpEvent(bool eventPassed) {
  ...
  if ( m_overlapRegAcc ) {
    EL_CHECK("cleanUpEvent",m_overlapRegAcc->clearRegister());
  }
