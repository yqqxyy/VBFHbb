...
EL::StatusCode EventInfoHandler::initialize() {
  ...
  if(!m_triggerList.size()) Warning("EventInfoHandler::initialize()","No trigger defined when form back the trigger list!");
    ...
    for (const std::string & trigger : m_triggerList) {
      ...
      else if ( trigger == "HLT_tau25_medium1_tracktwoEF") m_trigDecorators["HLT_tau25_medium1_tracktwoEF"] = &Props::passHLT_tau25_medium1_tracktwoEF ;
      else if ( trigger == "HLT_tau35_medium1_tracktwoEF") m_trigDecorators["HLT_tau35_medium1_tracktwoEF"] = &Props::passHLT_tau35_medium1_tracktwoEF ;
      else if ( trigger == "HLT_tau25_mediumRNN_tracktwoMVA") m_trigDecorators["HLT_tau25_mediumRNN_tracktwoMVA"] = &Props::passHLT_tau25_mediumRNN_tracktwoMVA ;
      else if ( trigger == "HLT_tau35_mediumRNN_tracktwoMVA") m_trigDecorators["HLT_tau35_mediumRNN_tracktwoMVA"] = &Props::passHLT_tau35_mediumRNN_tracktwoMVA ;
...
EL::StatusCode EventInfoHandler::setTruthWZProps(const xAOD::JetContainer* truthWZJets) {

  // Set props for the number of TruthWZ jets and the HT variable
  // Needed to reweight Sherpa 2.2, we store it for all samples
  // https://gitlab.cern.ch/atlas/athena/-/blob/21.2/PhysicsAnalysis/AnalysisCommon/PMGTools/Root/PMGSherpa22VJetsWeightTool.cxx
...
EL::StatusCode EventInfoHandler::isVZcc(bool isVZcc){
  Props::isVZcc.set(m_eventInfoIn, isVZcc);
  return EL::StatusCode::SUCCESS;
}
...
void EventInfoHandler::writeOutputVariables(xAOD::EventInfo* eventInfoIn, xAOD::EventInfo* eventInfoOut) {
  ...
  if ( m_isMC ) {
    ...
    Props::isVZcc.copyIfExists(eventInfoIn, eventInfoOut);
  ...
  Props::DFCommonJets_isBadBatman.copyIfExists(eventInfoIn, eventInfoOut);
  Props::DFCommonJets_eventClean_LooseBad.copy(eventInfoIn, eventInfoOut);
  Props::isCrackVetoClean.copy(eventInfoIn, eventInfoOut);
  Props::NTruthWZJets20.copyIfExists(eventInfoIn, eventInfoOut);
  Props::NTruthWZJets30.copyIfExists(eventInfoIn, eventInfoOut);
  Props::SumPtTruthWZJets20.copyIfExists(eventInfoIn, eventInfoOut);
