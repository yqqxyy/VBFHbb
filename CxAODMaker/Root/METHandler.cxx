...
EL::StatusCode METHandler::initializeTools() {
  ...
  m_config.getif<bool>("DoMuonPFlowBugfix",m_doMuonPFlowBugfix);
  ...
  bool isAFII = m_eventInfoHandler.get_isAFII();
  ...
  TOOL_CHECK("METHandler::METSignificance", m_metSignif.setProperty("IsAFII",   isAFII));
  ...
  TOOL_CHECK("METHandler::METSignificance", m_metSignif_PU.setProperty("IsAFII",   isAFII));
  ...
  TOOL_CHECK("METHandler::METMaker",   m_metMaker.setProperty("DoMuonPFlowBugfix", m_doMuonPFlowBugfix));
  TOOL_CHECK("METHandler::METMaker",   m_metMaker.setProperty("JetRejectionDec","passFJVTLoose"));
  ...
  if (m_handlerName == "METTrack"){ //Configure the track met if we use it
    ...
    TOOL_CHECK("METHandler::METHandler", m_metSysTool->setProperty("ConfigPrefix",  "METUtilities/run2_13TeV/")); // set last recomandation path
  ...
  if (muons) {
    ...
    for(const auto& muo : *muons) {
      if(m_handlerName == "METMJMUTight"){
	     if((!m_selObjForMET || Props::forMETRebuild.get(muo))&& (Props::muonQuality.get(muo) < 2) && Props::isTightTrackOnly_VarRadIso.get(muo)) metMuo.push_back(muo);
      }
      else{
