  ...
  m_VHbbJetSelection(nullptr)
{
    ...
    m_VHbbJetSelection = new VHbbJetSelection(&m_config);
}
...
void JetRegression::applyRegression(){
	...
	if (m_training) {
		...
		const std::string truthBosonsContainer=m_truthprocessor->getTruthBosonsContainer();
		truthParticles = m_truthprocessor->getInParticle(truthBosonsContainer);
		...
		for (const xAOD::Jet *jet : *jets) {
			...
			if(m_VHbbJetSelection->isSignalJet(jet)){
