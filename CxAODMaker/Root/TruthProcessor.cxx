    ...
    m_doFiducial(false)
{
  ...
  std::string truthBosonName= "TruthBosonWithDecayParticles";
  std::string truthBosonsName="TruthBosonsWithDecayParticles"; 

  if ( m_event->contains<xAOD::TruthParticleContainer>(truthBosonName) ) {
    m_truthBosonsContainer=truthBosonName;
  } else if ( m_event->contains<xAOD::TruthParticleContainer>(truthBosonsName) ) {
    m_truthBosonsContainer=truthBosonsName;
  } else {    
    std::cout << "Event does not contain the TruthBosons" << std::endl;
    Warning("TruthProcessor::TruthProcessor","The truth boson container is not found");
    m_truthBosonsContainer="TruthBosonContainerNotFound";
  }
  Info("TruthProcessor::TruthProcessor","Setting m_truthBosonsContainer name to: %s", m_truthBosonsContainer.c_str());
  m_listOfContainers.insert(m_truthBosonsContainer);

  // TruthTopQuarkWithDecayParticles in HIGGXXX but TruthTop in DOAD_PHYS
  std::string truthTopQuarkWithDecayParticlesName="TruthTopQuarkWithDecayParticles";
  std::string truthTopName= "TruthTop";

  if ( m_event->contains<xAOD::TruthParticleContainer>(truthTopQuarkWithDecayParticlesName) ) {
    m_truthTopContainer=truthTopQuarkWithDecayParticlesName;
  } else if ( m_event->contains<xAOD::TruthParticleContainer>(truthTopName) ) {
    m_truthTopContainer=truthTopName;
  } else {
    std::cout << "Event does not contain the TruthTop" << std::endl;
    Warning("TruthProcessor::TruthProcessor","The truth top container is not found");
    m_truthTopContainer="TruthTopContainerNotFound";
  }
  Info("TruthProcessor::TruthProcessor","Setting m_truthTopContainer name to: %s", m_truthTopContainer.c_str());
  m_listOfContainers.insert(m_truthTopContainer);

  // old style TruthParticles
  if ( m_event->contains<xAOD::TruthParticleContainer>("TruthParticles") ) {
    m_listOfContainers.insert("TruthParticles");
  }

  // flag to get additional truth info
  m_config.getif<bool>("doFiducial",m_doFiducial);
...
EL::StatusCode TruthProcessor::getTTBarDecayFromTruthTop(int & codeTTBarDecay, float &final_pt, float &final_ptl, float &final_etal) {
  ...
  const xAOD::TruthParticleContainer *TruthTop = m_in[m_truthTopContainer];
  const xAOD::TruthParticleContainer *TruthBoson = m_in[m_truthBosonsContainer];
...
bool TruthProcessor::checkSherpaVZqqZbb() {
  std::string contName = "TruthParticles";
  if (m_newStyle)
    contName = m_truthBosonsContainer;
...
bool TruthProcessor::checkSherpaVZqqZcc() {
  std::string contName = "TruthParticles";
  if (m_newStyle)
    contName = m_truthBosonsContainer;
  ...
  for (const xAOD::TruthParticle *part : *getInParticle(contName)) {
    if (part->status() == 11) {
      ...
      bool isWlv_v = (abs(id) == 12 || abs(id) == 14 || abs(id) == 16);
      // to do: check the impact when adding also negative values for the id of quarks
      bool isZqq = (id == 1 || id == 2 || id == 3 || id == 5);
      bool isZcc = (id == 4);
      if (isZll) nZll++;
      if (isZantill) nZantill++;
      if (isZvv) nZvv++;
  ...
  bool isZvv = ((nZvv == 1) && (nZantivv == 1));
  bool isWlv = ((nWlv_l == 1) && (nWlv_v == 1));
  bool isVZcc = ((isZll || isZvv || isWlv) && nZcc == 1);
  return isVZcc;
...
bool TruthProcessor::passCustomTruthParticle(xAOD::TruthParticle * part, const std::string &containerName) {
  ...
  if (m_newStyle) {
    ...
    if ( (containerName != "TruthHFWithDecayParticles") &&
         (containerName != m_truthBosonsContainer) &&
         (containerName != m_truthTopContainer) ) {
      setPassPresel(part);
      return true;
    }
    return false;
  ...
  // Diboson
  // 1) Identify Sherpa VZbb, VZcc events
...
EL::StatusCode TruthProcessor::writeOutputVariables(xAOD::TruthParticle * inPart, xAOD::TruthParticle * outPart) {
  ...
  if(m_doFiducial && (inPart->pdgId() == 11 || inPart->pdgId() == 13)){
    outPart->auxdata<float>("etcone20") = Props::etcone20.get(inPart);
    outPart->auxdata<float>("ptcone30") = Props::ptcone30.get(inPart);
  }
