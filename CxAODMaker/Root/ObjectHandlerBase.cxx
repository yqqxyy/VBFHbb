  ...
  m_saveNonCrucialVariables(false)
{
  ...
  m_config.getif<bool>("isDAODPHYS",m_isDAODPHYS);
  ...
  m_config.getif<std::string>("btagTimeStamp",m_btagTimeStamp);
  if ( m_btagTimeStamp!="none" ){
    if ( m_containerName=="AntiKt4EMTopoJets"
	 || m_containerName=="AntiKt4EMPFlowJets"
	 || m_containerName=="AntiKtVR30Rmax4Rmin02TrackJets"
	 ){
      m_containerName+=m_btagTimeStamp;
      Info("ObjectHandler::ObjectHandler()", "Combined container name '%s'.", m_containerName.c_str());
    }
  }
...
EL::StatusCode ObjectHandlerBase::addCPVariations(const std::vector<TString> &variations,
  ...
  if (jetUnc_FJJMR_configs.size() != jetUnc_FJJMR_prefixes.size())
    jetUnc_FJJMR_prefixes.clear();
  ...
  if (jetTagUnc_FJ_configs.size() != jetTagUnc_FJ_prefixes.size())
    jetTagUnc_FJ_prefixes.clear();
  ...
  allPrefixes.insert(allPrefixes.end(), jetUnc_FJJMR_prefixes.begin(), jetUnc_FJJMR_prefixes.end());
  allPrefixes.insert(allPrefixes.end(), jetTagUnc_FJ_prefixes.begin(), jetTagUnc_FJ_prefixes.end());
  ...
  for (const TString &name : variations) {
    ...
    if (filterByTools) {
      ...
      bool doFullJER = false;
      m_config.getif<bool>("doFullJER",doFullJER);
      bool smearLRPD = false;
      m_config.getif<bool>("smearLRPD",smearLRPD);
      /* 
       * JET_JERMC_XXX and JET_JERPD_XXX are added in config file
       * The name is not resolved by the JetUncertaintyTool
       * In order to pass the tool->isAffectedBySystematic check
       * the "MC" and "PD" must be removed in nameCopy
       */
      if (doFullJER || smearLRPD) {
	if(std::string(name.Data()).find("FATJET_CR_JET_JERPD") != std::string::npos) {
	  // FATJET_CR_J E T _ J E R P D
	  // 0123456789101112131415161718
	  int iJerStart = 17, iJerSize = 2;
	  nameCopy.erase(iJerStart, iJerSize);
	}else if (std::string(name.Data()).find("JET_JERMC_") != std::string::npos || std::string(name.Data()).find("JET_JERPD_") != std::string::npos) {
          // JET_JER[XY]_...
          // 0123456[78]9
