...
EL::StatusCode ForwardElectronHandler::decorateOriginParticle(const xAOD::Electron * electron)
{
  ...
  isLooseLH = electron -> isAvailable<char>("DFCommonForwardElectronsLHLoose") ? static_cast<int>(electron -> auxdata<char>("DFCommonForwardElectronsLHLoose")) : static_cast<bool>(m_checkLooseLH->accept(electron));
  if(isLooseLH) {
    isMediumLH = electron -> isAvailable<char>("DFCommonForwardElectronsLHMedium") ? static_cast<int>(electron -> auxdata<char>("DFCommonForwardElectronsLHMedium")) : static_cast<bool>(m_checkMediumLH->accept(electron));
    if(isMediumLH) {
      isTightLH = electron -> isAvailable<char>("DFCommonForwardElectronsLHTight") ? static_cast<int>(electron -> auxdata<char>("DFCommonForwardElectronsLHTight")) : static_cast<bool>(m_checkTightLH->accept(electron));
    }
  }
...
EL::StatusCode ForwardElectronHandler::writeOutputVariables(xAOD::Electron * inElectron, xAOD::Electron * outElectron, bool isKinVar, bool isWeightVar, const TString& sysName) 
{
  if (isKinVar || isWeightVar) {
    ...
    EL_CHECK("writeOutputVariables",writeAllVariations(inElectron, outElectron, sysName));
  } 
  if (isKinVar && !isWeightVar) {
    // Nominal Not, Kinematic Yes, Weight Not
    EL_CHECK("writeOutputVariables",writeKinematicVariations(inElectron, outElectron, sysName));
  } else if (!isKinVar && isWeightVar) {
    // Nominal Not, Kinematic Not, Weight Yes  
    EL_CHECK("writeOutputVariables",writeWeightVariations(inElectron, outElectron, sysName));
  } else if (!isKinVar && !isWeightVar) {
    ...
    EL_CHECK("writeOutputVariables",writeKinematicVariations(inElectron, outElectron, sysName));
    EL_CHECK("writeOutputVariables",writeWeightVariations(inElectron, outElectron, sysName));
    EL_CHECK("writeOutputVariables",writeNominal(inElectron, outElectron, sysName));
  } else assert(false);
