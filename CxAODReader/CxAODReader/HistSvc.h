...
public:
  ...
  void BookFillHist(const string &name, int nbinsx, double *xbins, int nbinsy,
                    double ylow, double yup, float xvalue, float yvalue,
                    float weight = 1,
                    doSysts ds = doSysts::yes); // TH2 (2D)
  ...
  // If a global m_fillAllSysts flag is false, the IsHistForSystematics function
  // is called to determine if syst variations need to be stored for a histogram
  // fullname. It should be redefined in derived classes (e.g. to get a list of
  // such histograms from a text file).
