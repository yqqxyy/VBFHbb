...
// TODO super important: add a check at the end that we never have several
// regions with same name but different integer codes

template <typename RegNaming> class HistFastSvc {
private:
...
public:
  HistFastSvc() = default;
  ~HistFastSvc() = default; // to be revisited some day by taking care of
                            // deallocating histos properly
  ...
  /// Write all histos to file. How histos are renamed, or placed into
  /// subdirectories, is delegated to RegNaming
  void Write(TFile *file);

  /// Register all histos to EventLoop. How histos are renamed, or placed into
  /// subdirectories, is delegated to RegNaming
