...
MVATree::MVATree(bool persistent, bool readMVA, EL::IWorker *wk,
    ...
    : m_persistent(persistent), m_readMVA(readMVA), m_nominalOnly(nominalOnly),
      m_treeMap(), m_currentVar(""), m_reader() {
...
MVATree::MVATree(bool persistent, bool readMVA, EL::IWorker * /*wk*/)
    : m_persistent(persistent), m_readMVA(readMVA), m_nominalOnly(true),
      m_treeMap(), m_currentVar(""), m_reader() {}
...
void MVATree::Fill() {
  ...
  m_iterator_treeMap = m_treeMap.find(m_currentVar);
  if (m_iterator_treeMap == m_treeMap.end()) {
    Error("MVATree::Fill()", "No tree registered for variation '%s'",
          m_currentVar.c_str());
    exit(EXIT_FAILURE);
  }

  m_iterator_treeMap->second->Fill();
}
