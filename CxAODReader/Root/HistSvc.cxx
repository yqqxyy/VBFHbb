...
HistSvc::HistSvc()
    : m_bootstraphists(), m_fillHists(true), m_fillAllSysts(true),
      m_nameSvc(nullptr),
      m_bootstrap(new BootstrapGenerator("gen", "gen", 1000)),
      m_weightSysts(nullptr), m_weightSystsCS(nullptr) {
...
void HistSvc::BookFillHist(const string &name, int nbinsx, double *xbins,
  ...
  float weightSys = 0 ; 
  for (const WeightSyst & wS : *m_weightSysts){
    weightSys = weight * wS.factor;
    m_nameSvc->set_variation(wS.name);
    static_cast<TH2F*>(BookHist(name, nbinsx, xbins, nbinsy, ylow, yup))
        ->Fill(xvalue, yvalue, weightSys);
  }

  for (const WeightSyst & wS : *m_weightSystsCS){
    weightSys = weight * wS.factor;
    m_nameSvc->set_variation(wS.name);
    static_cast<TH2F*>(BookHist(name, nbinsx, xbins, nbinsy, ylow, yup))
        ->Fill(xvalue, yvalue, weightSys);
...
void HistSvc::Write(EL::IWorker *wk) {
  ...
  for (auto entry : m_bootstraphists) {
    ...
    int pos = entry.first.find("2tag");
    for (int i = 0; i < 1000; i++) {
      TH1 *hist = bootstraphist->GetReplica(i);
      std::string name = entry.first;
      name.insert(pos - 1, std::to_string(i));
...
bool HistSvc::IsHistForSystematics(const string & /*fullname*/) {
  return false;
}
