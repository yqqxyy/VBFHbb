...
Histo::Histo(const Histo &other) noexcept
    : m_h(other.m_h ? (TH1 *)(other.m_h->Clone()) : nullptr),
      m_h2(other.m_h2 ? (TH2 *)(other.m_h2->Clone()) : nullptr), m_w(other.m_w),
      m_x(other.m_x), m_y(other.m_y), m_getx(other.m_getx),
      m_gety(other.m_gety) {
  /*
  std::cout << "In copy ctor" << std::endl;
  if(!other.m_h) { std::cout << "other nullptr ! something is weird" <<
  std::endl; } else if(!m_h) { std::cout << "this nullptr ! something is weird"
  << std::endl; } else { std::cout << m_h->GetName() << std::endl;}
  */
}
...
Histo::Histo(TH1 *h, std::function<double()> x, double &w) noexcept
    : m_h(h), m_h2(nullptr), m_w(w), m_x(nullptr), m_y(nullptr), m_getx(x),
      m_gety() {
  m_h->Sumw2();
}
...
Histo::Histo(TH2 *h, std::function<double()> x, std::function<double()> y,
             double &w) noexcept
    : m_h(nullptr), m_h2(h), m_w(w), m_x(nullptr), m_y(nullptr), m_getx(x),
      m_gety(y) {
  m_h2->Sumw2();
}
